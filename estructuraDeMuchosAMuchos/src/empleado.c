...
void mostrarMascota(eMascota empleado, eColor sector[], int len)//
{
	...
	if(cargarDescripcionColor(empleado.sector, sector,len, descripcion)==0)
	{
		...
		while(respuesta!='n')
		{
			if(buscarLibreMascota(lista, len, &indice)==0)
			{
				...
				if(utn_getString(aux.nombre, "Ingrese nombre", "ERROR reingrese nombre", 5)==0)
				{
					...
					{
						if(mostrarArrayColore(sector, lenSector)==0)
						{
...
int listarEmpleadosColor(eColor sector[], int lenSec, eMascota lista[], int lenEmp)
{
	...
	{
		if(mostrarArrayColore(sector, lenSec)==0)
		{
			if(utn_getNumeroEnteroConIntentos(&respuesta, "Ingrese 1-listar empleados por sector 2- mostrar total de sueldos por sector ", "ERROR REINGRESE OPCION", 1, 2, 5)==0)
			{
				if(utn_getNumeroEnteroConIntentos(&opcion, "Ingrese el sector a listar empleados ", "ERROR el sector no existe ", 500, 504, 5)==0)
				{
					...
					{
						case 1:
							if(recorrerYMostrarListaColor(sector, lenSec, lista, lenEmp, opcion,&total)==0)
							{
								error=0;
							}
